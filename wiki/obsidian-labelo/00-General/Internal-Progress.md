...
---
## Summary
Teams, projects, and kanban labels are implemented. Label JSON import/export with AJV validation is available on the project board. Projects can be deleted (with confirm); long label titles wrap. Inputs stay at **17px**. PWA install + iOS pull-to-refresh ported from Famity Care patterns.
...
- Done: [[Delete-Projects-Labels]] — `deleteProjectAction` + `ResponsiveMenu` (drawer on mobile); flow cards `w-[220px]` with `break-words` wrap (avoids character-column collapse).
