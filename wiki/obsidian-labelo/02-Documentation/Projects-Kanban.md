...
## Actions
Labels have a title only; titles may duplicate. Flow cards support **duplicate**, **change status**, and **delete**. Backlog rows support **move to requerimiento** and **delete**. Mutations use server actions under `lib/projects/` and `lib/tasks/` (no `app/api` routes yet).
