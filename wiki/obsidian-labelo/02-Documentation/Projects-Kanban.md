...
## Safe area
The status rail floats above the slides, so `.label-safe-area` (in `app/globals.css`) clears its 58.5px track (13px offset plus a 45.5px rail) plus a 13px gutter — 71.5px — on **both** sides. The content box stays symmetric, so centered titles sit in the middle of the viewport with equal room on each edge instead of running under the dots. Slides with no rail (a lone project on home) set `data-rail="none"`, leaving a plain 13px gutter.
